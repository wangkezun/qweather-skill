...
2. **城市查询**：如果用户给的是城市名，先调用 `node scripts/api.js city-lookup` 获取 `location ID`
3. **调用对应 API**：使用 `node scripts/api.js <command>` 调用相应接口（自动处理 JWT 认证）
4. **格式化输出**：按模板呈现结果
...
```bash
# 城市查询
node scripts/api.js city-lookup --location=北京

# 实时天气
node scripts/api.js weather-now --location=101010100

# 逐日预报（默认3天，可选 3/7/10/15/30）
node scripts/api.js weather-daily --location=101010100 --days=7

# 逐时预报（默认24小时，可选 24/72/168）
node scripts/api.js weather-hourly --location=101010100

# 空气质量
node scripts/api.js air-quality --lat=39.90 --lon=116.41

# 灾害预警
node scripts/api.js weather-alert --lat=39.90 --lon=116.41

# 生活指数（type: 0=全部, 3=穿衣, 5=紫外线, 9=感冒）
node scripts/api.js indices --location=101010100 --type=3,5,9

# 分钟级降水（仅中国大陆）
node scripts/api.js minutely --lat=39.90 --lon=116.41

# 日出日落
node scripts/api.js sun --location=101010100 --date=20260415

# 月升月落
node scripts/api.js moon --location=101010100 --date=20260415
```
